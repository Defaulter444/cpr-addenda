...
async function buildPack(packName, packType = "Item") {
  ...
  // Свежезаписанное LevelDB держит в журнале — файле вида `000171.log`, — и
  // переносит в `.ldb` только при уплотнении. Для нас это не мелочь: пак
  // уезжает в репозиторий и в архив релиза целой папкой, а всё, что осталось
  // в журнале, туда не попадало. Уплотняем сами, чтобы данные лежали в
  // `.ldb`, а журнал был пуст.
