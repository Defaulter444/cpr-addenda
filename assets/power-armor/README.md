...
Имя файла должно совпадать с именем костюма. Инструмент приводит регистр и
пробелы, понимает кириллические двойники латиницы («АRASAKA» через русскую А) и
полную транслитерацию («СОВОЙЛ БОМБАРДИР», «ZHIRAFA БОРИС»). Непонятные файлы
не трогает и перечисляет.
