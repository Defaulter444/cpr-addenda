 ...
 * повёрнут стрелок, решает игрок: шаблон можно подвинуть мышью, а кнопка
 * «Пересчитать» на карточке обновит список попавших.
 *
...
export async function placeArea({ item, actor, kind, attackTotal }) {
  ...
  if (kind === BLAST) {
    ...
    // повёрнут токен. Направление округляется до ближайшей из восьми сторон:
    // блок идёт по клеткам, а клетки не поворачиваются. Если вышло не то,
    // игрок двигает шаблон мышью и жмёт «Пересчитать».
    if (target) {
...
/**
 * Пересчёт списка после того, как игрок передвинул зону.
 *
 * Зону двигают уже после выстрела — и дробь, и промахнувшийся взрыв,
 * который книга велит мастеру перенести. Поэтому список в
